...
- 💉 `ShortcutRegistry` is now the @Injectable service (replaces `KbdModule` static methods)
- 🏷️ Added DI tokens: `KBD_CONFIG`, `SHORTCUT_REGISTRY`
- 📁 Added `constants/` folder with `tokens.constant.ts`
- 🔄 `KbdModule.register()` / `.get()` / `.getAll()` / `.query()` replaced by `ShortcutRegistry` methods
