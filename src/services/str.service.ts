...
export class Str {
  ...
  static camel(value: string): string {
    // Studly first (each word capitalized), then lowercase the first char.
    return Str.studly(value).replace(/^(.)/, (char) => char.toLowerCase());
  }
  ...
  static splitWords(value: string): string[] {
    ...
    return (
      value
        // Insert a space between an uppercase run and a following uppercase+lowercase.
  ...
  static swap(map: Record<string, string>, subject: string): string {
    ...
    // Build a single regex matching any of the search keys (longest first
    // to avoid partial overlaps), then replace via lookup table.
